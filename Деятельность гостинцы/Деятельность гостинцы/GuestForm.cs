...
namespace Деятельность_гостинцы
{
    public partial class GuestForm : Form
    {
        ...
        public void RemoveReservedRoom(int roomNumberToRemove)
        {
            if (dataGridView1.Columns.Contains("Number"))
            {
                DataGridViewColumn numberColumn = dataGridView1.Columns["Number"];
                int columnIndex = numberColumn.Index;

                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Cells[columnIndex].Value != null && Convert.ToInt32(row.Cells[columnIndex].Value) == roomNumberToRemove)
                    {
                        dataGridView1.Rows.Remove(row);
                        UpdateComboBox(roomNumberToRemove);
                        break;
                    }
                }
            }
            ...
            {
                MessageBox.Show("Столбец 'Number' не найден в DataGridView.", "Ошибка удаления");
            }
        }
        private void UpdateComboBox(int roomNumberToRemove)
        {
            if (comboBox1.InvokeRequired)
            {
                comboBox1.Invoke(new Action(() => UpdateComboBox(roomNumberToRemove)));
            }
            else
            {
                if (comboBox1.Items.Contains(roomNumberToRemove))
                {
                    comboBox1.Items.Remove(roomNumberToRemove);
                }
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string defaultQuery = "SELECT Number, Rooms, Storey, TV, Fridge, Bed, Type, Balcony FROM Rooms";
            ...
            {
                string query = "SELECT Number, Rooms, Storey, TV, Fridge, Bed, Type, Balcony FROM Rooms WHERE 1=1 ";
                if (storey > 0)
                {
