...
namespace Проект
{
    public partial class Passport : Form
    {
           ...
           for (int j = 0; j < dataGridView2.RowCount; j++)
            {
                ...
                {
                    if (dataGridView2.Rows[j].Cells[0].Value.ToString().Contains(textBox4.Text))
                    { 
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            sqlConnect.Open();
            SqlDataAdapter da = new SqlDataAdapter($"DELETE FROM  Удостверение_личности  WHERE Код_Удовстверения = {label11.Text}; ", sqlConnect);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            SqlConnection sqlConnect = new SqlConnection("Data Source=sql;Initial Catalog = уП_ПМ01_ИСПП_5_Степурин_НС; Integrated Security = True");
            sqlConnect.Open();
            SqlDataAdapter da = new SqlDataAdapter($"INSERT   INTO   Удостверение_личности(Должность,Звание_военнослужащего,Вид_службы) VALUES('{textBox1.Text} ','{textBox2.Text }','{ textBox3.Text}');", sqlConnect); ;
            DataTable dt = new DataTable();
            da.Fill(dt);
            удостверение_личностиTableAdapter.Fill(уП_ПМ01_ИСПП_5_Степурин_НСDataSet.Удостверение_личности);
        }
